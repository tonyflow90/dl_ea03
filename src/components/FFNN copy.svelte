<script>
    ...
    let saveModel = async () => {
        const saveResult = await model.save("localstorage://ffnn-model");
    };

    let loadModel = async () => {
        const model = await tf.loadLayersModel("localstorage://ffnn-model");
        return model;
    };
    ...
    let getData2 = () => {
        let dataArray = [];
        for (let i = 0; i < 1; i += 0.001) dataArray.push({ x: i, y: calc(i) });
        return dataArray;
    };

    let getData = (nCount = 1000) => {
        let dataArray = [];
        for (let i = 0; i < 1; i += 1 / nCount)
            dataArray.push({ x: i, y: calc(i) });
        return dataArray;
    };
